...
function collectDialog () {
  ...
  const dd = `
      ...
      ${
        type === 'single' ?
        '<span>當前頁支持單品採集，請點擊搬家按鈕，採集商品</span>':
        type === 'batch' ?
        '<span>當前頁支持批量採集，請</span><span style="color: #DC3545">勾選商品</span><span>後，再點擊搬家按鈕</span>': ''
      }
      <button id="collectDialog-button">一鍵搬家</button>
      </div>
      `

  const dom = `
    <div class="momo-collect-dialog">
      <div id="momo-content" class="momo-fold-content">
        <div id="momo-warehouse">
  ...
  // const collectDialogButton = document.getElementById('collectDialog-button')
  // collectDialogButton.addEventListener('click', () => {
